class KkuTactic < ActiveRecord::Base
    scope :all_enabled, -> { order("kku_strategics.no, kku_strategics.name, kku_tactics.no, kku_tactics.name").includes(:kku_strategic).where(["kku_tactics.workflow_state IN (?)", self.enabled_states]) }
    ...
    validates :kku_strategic_id, presence: true
    ...
    def to_s_long
      "#{KkuTactic.human_attribute_name(:no)} #{no} #{name}"
    end
    
    def to_s_short
      "#{KkuTactic.human_attribute_name(:no)} #{no}"
    end
    
    def to_s_sort
      "#{kku_strategic.to_s_sort} #{"%05d" % no} #{name}"
    end
