class AllowanceRate < ActiveRecord::Base
  scope :all_enabled, -> { order("sub_budget_types.no, sub_budget_types.name, allowance_rates.no, allowance_rates.name").includes(:sub_budget_type).where(["allowance_rates.workflow_state IN (?)", self.enabled_states]) }
  ...
  def to_s_sort
    "#{sub_budget_type.to_s_sort} #{"%05d" % no} #{name}"
  end
