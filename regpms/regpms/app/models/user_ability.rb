class UserAbility
  ...
  def initialize(user)
    ...
    if user
      ...
      # Project
      can [:read], Project
      can [:update, :save_change, :save_change_as_draft, :change_to_draft, :finish, :reopen, :print, :chose_strategy, :chose_strategy_group, :chose_tactic, :chose_kku_strategic_pillar,:chose_kku_strategic], Project do |project|
      ...
      can [:read], Budget
      can [:update, :chose_budget_type, :chose_sub_budget_type],Budget
      ...
      if user.authorized_as?(:accounting) || user_group.authorized_as?([:accounting])
        ...
        can [:cru, :submit, :save_change, :enable, :disable, :terminate], SubBudgetType
        ...
        can [:cru, :submit, :save_change, :enable, :disable, :terminate], AllowanceRate
        can [:cru, :submit, :save_change, :enable, :disable, :terminate], WageRate

        can [:cru, :submit, :save_change, :enable, :disable, :terminate, :chose_project, :chose_budget_type, :chose_sub_budget_type, :import, :import_save], Expense
        #:chose_sub_budget_type
      ...
      if user.authorized_as?(:planner) || user_group.authorized_as?([:planner])
        ...
        can [:cru, :submit, :save_change, :enable, :disable, :terminate], KkuTactic
      ...
      can [:read, :user, :confirm_job_plan, :confirm_job_routine, :confirm_job_vice, :confirm_job_development, :unconfirm_job_plan, :unconfirm_job_routine, :unconfirm_job_vice, :unconfirm_job_development, :confirm_job_plan_result, :confirm_job_routine_result, :confirm_job_vice_result, :confirm_job_development_result, :unconfirm_job_plan_result, :unconfirm_job_routine_result, :unconfirm_job_vice_result, :unconfirm_job_development_result, :confirm_selected], :confirm do
        Section.where(["leader_id = ? OR vice_leader_id = ?", user.id, user.id]).size > 0 || Evaluation.all_enabled.collect {|ev| ev.director_id}.include?(user.id) || Evaluation.all_enabled.collect {|ev| ev.vice_director_id}.include?(user.id)
      ...
      if user.is_director? || user.is_vice_director? # || user.is_section_leader?
        can :read, :report
        can :users, :report
        can :user, :report
        can :comment, :report
