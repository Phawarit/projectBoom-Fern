...
class Project < ActiveRecord::Base
  ...
  # validates_presence_of :kku_tactic, :kku_strategic, :kku_strategic_pillar, :strategy, :tactic, :strategy_group
  ...
  def total_expense_with_children_percent(ddd=nil)
    percent = 0.to_f
    if budget_amount.to_f.to_f != 0
      percent = total_expense_with_children(ddd).to_f / budget_amount.to_f.to_f * 100.0
    end
    return percent
  end

  def total_percent_of_task
    percent = 0
    problem_suggesstions.each do |pb|
      percent += pb.percent_of_task
    end
    return percent
  end

  def total_percent_of_task_percent(ddd=nil)
    100.0 - total_percent_of_task.to_f
  end

  def check_percent_of_task_to_change_state
    percent = 0
    problem_suggesstions.each do |pb|
      percent += pb.percent_of_task
    end
    if percent == 100
      pj = Project.find(id)
      pj.update_column(:workflow_state, "finished")
      reload
