...
class User < ActiveRecord::Base
  ...
  def assess_to(ev = nil, role = nil, options = {})
    if ev.nil? || role.blank?
      ...
      if role == "director"
        ...
        where = ["sections.vice_director_id = ? AND sections.workflow_state='enabled'", id]
        users = User.joins(joins).where(where)
      elsif role == "vice_director2"
        joins = "JOIN section_users ON users.id = section_users.user_id "
        joins += "JOIN sections ON sections.id = section_users.section_id "
        where = ["sections.vice_director2_id = ? AND sections.workflow_state='enabled'", id]
        users = User.joins(joins).where(where)
      elsif role == "vice_director3"
        joins = "JOIN section_users ON users.id = section_users.user_id "
        joins += "JOIN sections ON sections.id = section_users.section_id "
        where = ["sections.vice_director3_id = ? AND sections.workflow_state='enabled'", id]
        users = User.joins(joins).where(where)
        ...
        joins = "JOIN section_users ON users.id = section_users.user_id "
        joins += "JOIN sections ON sections.id = section_users.section_id "
        where = ["section_users.section_id = ? AND sections.workflow_state='enabled'", options[:section_id]]
  ...
  def assess_vice_director_by(ev = nil)
    ...
    if sections.select { |sss| sss.name.include?("รองผู้อำนวยการ") }.size > 0
      sections.select { |sss| sss.name.include?("รองผู้อำนวยการ") }.each do |s|
        ls << s.vice_director if !s.vice_director.nil? && s.vice_director != id
      end if !is_vice_director2?(ev) && !is_vice_director3?(ev) && !is_secretary?(ev)
    else
      sections.select { |sss| !sss.name.include?("รองผู้อำนวยการ") }.each do |s|
        ls << s.vice_director if !s.vice_director.nil? && s.vice_director != id
      end if !is_vice_director2?(ev) && !is_vice_director3?(ev) && !is_secretary?(ev)
  ...
  def assess_vice_director2_by(ev = nil)
    ...
    if sections.select { |sss| sss.name.include?("รองผู้อำนวยการ") }.size > 0
      sections.select { |sss| sss.name.include?("รองผู้อำนวยการ") }.each do |s|
        ls << s.vice_director2 if !s.vice_director2.nil? && s.vice_director2 != id
      end if !is_vice_director?(ev) && !is_vice_director3?(ev) && !is_secretary?(ev)
    else
      sections.select { |sss| !sss.name.include?("รองผู้อำนวยการ") }.each do |s|
        ls << s.vice_director2 if !s.vice_director2.nil? && s.vice_director2 != id
      end if !is_vice_director?(ev) && !is_vice_director3?(ev) && !is_secretary?(ev)
  ...
  def assess_vice_director3_by(ev = nil)
    ...
    if sections.select { |sss| sss.name.include?("รองผู้อำนวยการ") }.size > 0
      sections.select { |sss| sss.name.include?("รองผู้อำนวยการ") }.each do |s|
        ls << s.vice_director3 if !s.vice_director3.nil? && s.vice_director3_id != id
        #end if !is_vice_director?(ev) && !is_secretary?(ev)
      end if !is_vice_director?(ev) && !is_vice_director2?(ev) && !is_secretary?(ev)
    else
      sections.select { |sss| !sss.name.include?("รองผู้อำนวยการ") }.each do |s|
        ls << s.vice_director3 if !s.vice_director3.nil? && s.vice_director3_id != id
  ...
  def task_leader_total(ev = nil)
    if ev.nil?
      ...
      total_score_real_d = 0
      assess_director_by(ev).each do |ab|
        total_score_real = committee_employee_type_task_users.select { |ettu| ettu.committee_id == ab.id && !ettu.role.blank? && ettu.role == "director" }.collect { |ettu| ettu.score_real.to_f }.sum
        total_score_real_d = total_score_real
        total += total_score_real
        count += 1 if total_score_real > 0
      end
      scorexd = total_score_real_d
      #count > 0 ? (total / count) : 0
      # logger.info "SCORE_XD #{scorexd}"

      total_score_real_vd = 0
      assess_vice_director_by(ev).each do |ab|
        total_score_real = committee_employee_type_task_users.select { |ettu| ettu.committee_id == ab.id && !ettu.role.blank? && ettu.role == "vice_director" }.collect { |ettu| ettu.score_real.to_f }.sum
        total_score_real_vd = total_score_real
        total += total_score_real
        count += 1 if total_score_real > 0
      end
      scorexvd = total_score_real_vd
      #count > 0 ? (total / count) : 0
      # logger.info "SCORE_XVD #{scorexvd}"

      # assess_vice_director_by(ev).each do |ab|
      #   total_score_real = committee_employee_type_task_users.select { |ettu| ettu.committee_id == ab.id && !ettu.role.blank? && ettu.role == "vice_director" }.collect { |ettu| ettu.score_real.to_f }.sum
      #   logger.info "VD_ABID #{ab.id}"
      #   total += total_score_real
      #   count += 1 if total_score_real > 0
      # end
      # scorexvd = count > 0 ? (total / count) : 0
      total_score_real_vd2 = 0
      assess_vice_director2_by(ev).each do |ab|
        total_score_real = committee_employee_type_task_users.select { |ettu| ettu.committee_id == ab.id && !ettu.role.blank? && ettu.role == "vice_director2" }.collect { |ettu| ettu.score_real.to_f }.sum
        total_score_real_vd2 = total_score_real
        total += total_score_real
        count += 1 if total_score_real > 0
      end
      scorexvd2 = total_score_real_vd2
      #count > 0 ? (total / count) : 0
      # logger.info "SCORE_XVD2 #{scorexvd2}"

      total_score_real_vd3 = 0
      assess_vice_director3_by(ev).each do |ab|
        total_score_real = committee_employee_type_task_users.select { |ettu| ettu.committee_id == ab.id && !ettu.role.blank? && ettu.role == "vice_director3" }.collect { |ettu| ettu.score_real.to_f }.sum

        total_score_real_vd3 = total_score_real
        total += total_score_real
        # total = ab.id
        # logger.info "VD3_ABID #{ab.id}"
        count += 1 if total_score_real > 0
      end
      scorexvd3 = total_score_real_vd3 #count > 0 ? (total / count) : 0
      # logger.info "SCORE_XVD3 #{scorexvd3}"

      total_score_real_ss = 0
      assess_secretary_by(ev).each do |ab|
        total_score_real = committee_employee_type_task_users.select { |ettu| ettu.committee_id == ab.id && !ettu.role.blank? && ettu.role == "secretary" }.collect { |ettu| ettu.score_real.to_f }.sum
        total += total_score_real
        count += 1 if total_score_real > 0
        total_score_real_ss = total_score_real
      end
      scorexs = total_score_real_ss > 0 ? (total_score_real_ss) : 0
      #count > 0 ? (total / count) : 0
      # logger.info "SCORE_XSS #{scorexs}"

      total_score_real_sl = 0
      assess_section_leaders_by(ev).each do |ab|
        total_score_real = committee_employee_type_task_users.select { |ettu| ettu.committee_id == ab.id && !ettu.role.blank? && ettu.role == "section_leader" }.collect { |ettu| ettu.score_real.to_f }.sum
        total += total_score_real
        count += 1 if total_score_real > 0
        total_score_real_sl = total_score_real
        # logger.info "SL_ABID #{ab.id}"
      end
      scorexsl = total_score_real_sl > 0 ? (total_score_real_sl) : 0 #count > 0 ? (total / count) : 0
      # logger.info "SCORE_XSL #{scorexsl}"

      total_score_real_svl = 0
      assess_section_vice_leaders_by(ev).each do |ab|
        total_score_real = committee_employee_type_task_users.select { |ettu| ettu.committee_id == ab.id && !ettu.role.blank? && ettu.role == "section_vice_leader" }.collect { |ettu| ettu.score_real.to_f }.sum
        total += total_score_real
        count += 1 if total_score_real > 0
        total_score_real_svl = total_score_real
      end
      scorexsvl = total_score_real_svl
      #count > 0 ? (total / count) : 0
      ...
      scorex = ((scorexd * evet.director_ratio.to_f) + (scorexvd * evet.vice_director_ratio.to_f) + (scorexvd2 * evet.vice_director2_ratio.to_f) + (scorexvd3 * evet.vice_director3_ratio.to_f) + (scorexs * evet.secretary_ratio.to_f) + (scorexsl * evet.section_leader_ratio.to_f) + (scorexsvl * evet.section_vice_leader_ratio.to_f)) / leader_ratio(ev)
      # scorex = scorexvd3 #(scorexvd3 * evet.vice_director3_ratio.to_f / leader_ratio(ev))#(scorexsl * evet.section_leader_ratio.to_f / leader_ratio(ev))
      ...
      # return scorex, (scorexd * evet.director_ratio.to_f / leader_ratio(ev)), 0, 0, 0, 0, (scorexsl * evet.section_leader_ratio.to_f / leader_ratio(ev)), 0
    end
  end
  ...
  def ability_leader_total(ev = nil)
    if ev.nil?
      ...
      # logger.info "VD2_AB #{total_score_real_vd2}"
      scorexvd2 = total_score_real_vd2 #count > 0 ? (total / count) : 0
      ...
      scorexs = total_score_real_ss #count > 0 ? (total / count) : 0
      ...
      scorexsl = total_score_real_sl #count > 0 ? (total / count) : 0
      ...
      scorexsvl = total_score_real_svl #count > 0 ? (total / count) : 0
