class Projects::ProblemSuggesstionsController < OrbController
  ...
  def update
    ...
    respond_to do |format|
      if (params[:problem_suggesstion].nil? || @problem_suggesstion.update(problem_suggesstion_params)) && (!@problem_suggesstion.respond_to?(:current_state) || @problem_suggesstion.process_event!(params[:button]))
        format.html { redirect_to projects_problem_suggesstions_url(q: params[:q], page: params[:page], page: params[:page], q: params[:q]), notice: 'Problem suggesstion was successfully updated.' }
        format.js
  ...
  def problem_suggesstion_params
    params.require(:problem_suggesstion).permit(:workflow_state, :workflow_state_updater_id, :project_id, :sorting, :description, :next_step, :percent_of_task)
  end
