...
class ProjectsController < OrbController
  ...
  def print
    ...
    t = Time.zone.now
    filename = "project_#{@project.id}_#{@project.current_state}_#{t.strftime("%Y-%m-%d-%H-%M-%S")}.pdf"
    ...
    #pdf.number_pages "#{Time.now.strftime("%d/%m/%Y %H:%M:%S")}", :size => 9, :at => [500, 0]
    pdf.number_pages "#{t.strftime("%d/%m/%Y %H:%M:%S")}", { :start_count_at => 0, :page_filter => :all, :at => [pdf.bounds.right - 80, 0], :align => :right, :size => 11 }
  ...
  def chose_kku_strategic
    @kku_strategic = !params[:kku_strategic_id].blank? && KkuStrategic.exists?(params[:kku_strategic_id]) ? KkuStrategic.find(params[:kku_strategic_id]) : KkuStrategic.new
    respond_to do |format|
      format.js
    end
  end
  ...
  private
    ...
    params.require(:project).permit(
        ...
        :kku_strategic_pillar_id, :kku_strategic_id, :kku_tactic_id,
        ...
        responsibles_attributes: [:id, :workflow_state, :workflow_state_updater_id, :project_id, :sorting, :user_id, :prefix, :firstname, :lastname, :position, :responsibility, :percent_of_work, :_destroy],
        budgets_attributes: [:id,:workflow_state, :workflow_state_updater_id, :project_id, :sorting, :budget_type_id, :wage_rate_id ,:description, :amount ,:_destroy],
        ...
        problem_suggesstions_attributes: [:id, :workflow_state, :workflow_state_updater_id, :project_id, :sorting, :description, :next_step, :percent_of_task, :_destroy],
